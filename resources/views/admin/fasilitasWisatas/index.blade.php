            ...
            <table class=" table table-bordered table-striped table-hover datatable datatable-fasilitasWisata">
                <thead>
                    <tr>
                        ...
                        {{-- <th>
                            {{ trans('cruds.fasilitasWisata.fields.kode_fasilitasWisata') }}
                        </th> --}}
                        <th>
                            &nbsp;
                        </th>
                ...
                <tbody>
                    @foreach($fasilitasWisatas as $key => $fasilitasWisata)
                        <tr data-entry-id="{{ $fasilitasWisata->id }}">
                            ...
                            {{-- <td>
                                {{ $fasilitasWisata->kode_fasilitasWisata ?? '' }}
                            </td> --}}
                            <td>
                                @can('fasilitasWisata_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.fasilitasWisatas.show', $fasilitasWisata->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('fasilitasWisata_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.fasilitasWisatas.edit', $fasilitasWisata->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('fasilitasWisata_delete')
                                    <form action="{{ route('admin.fasilitasWisatas.destroy', $fasilitasWisata->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@endcan
  ...
  let table = $('.datatable-fasilitasWisata:not(.ajaxTable)').DataTable({ buttons: dtButtons })
