...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.dataprofils.update', [$dataprofil->id]) }}"
                ...
                <div class="form-group">
                    <label class=""
                        for="daftar_usaha_pariwisata">{{ trans('cruds.dataprofil.fields.daftar_usaha_pariwisata') }}</label>
                    <select class="form-control" name="daftar_usaha_pariwisata" id="daftar_usaha_pariwisata">
                        <option value="Hotel dan Akomodasi Lainnya"
                            {{ $dataprofil->daftar_usaha_pariwisata == 'Hotel dan Akomodasi Lainnya' ? 'selected' : '' }}>
                            Hotel dan Akomodasi Lainnya</option>
                        <option value="Daya Tarik Wisata"
                            {{ $dataprofil->daftar_usaha_pariwisata == 'Daya Tarik Wisata' ? 'selected' : '' }}>Daya Tarik
                            Wisata </option>
                        <option value="Restoran"
                            {{ $dataprofil->daftar_usaha_pariwisata == 'Restoran' ? 'selected' : '' }}>Restoran</option>
                        <option value="Kafe" {{ $dataprofil->daftar_usaha_pariwisata == 'Kafe' ? 'selected' : '' }}>Kafe
                        </option>
                        <option value="Usaha Makanan"
                            {{ $dataprofil->daftar_usaha_pariwisata == 'Usaha Makanan' ? 'selected' : '' }}>Usaha Makanan
                        </option>
                        <option value="Usaha Minuman"
                            {{ $dataprofil->daftar_usaha_pariwisata == 'Usaha Minuman' ? 'selected' : '' }}>Usaha Minuman
                        </option>
                        <option value="Bar, Klub Malam, dan Karaoke"
                            {{ $dataprofil->daftar_usaha_pariwisata == 'Bar, Klub Malam, dan Karaoke' ? 'selected' : '' }}>
                            Bar, Klub Malam, dan Karaoke</option>
                        <option value="SPA dan Rumah Pijat"
                            {{ $dataprofil->daftar_usaha_pariwisata == 'SPA dan Rumah Pijat' ? 'selected' : '' }}>SPA dan
                            Rumah Pijat</option>
                        <option value="Jasa Pariwisata"
                            {{ $dataprofil->daftar_usaha_pariwisata == 'Jasa Pariwisata' ? 'selected' : '' }}>Jasa
                            Pariwisata</option>
                        <option value="Perkemahan dan Pondok Wisata"
                            {{ $dataprofil->daftar_usaha_pariwisata == 'Perkemahan dan Pondok Wisata' ? 'selected' : '' }}>
                            Perkemahan dan Pondok Wisata</option>
                        <option value="Taman Rekreasi dan Hiburan"
                            {{ $dataprofil->daftar_usaha_pariwisata == 'Taman Rekreasi dan Hiburan' ? 'selected' : '' }}>
                            Taman Rekreasi dan Hiburan</option>
                        <option value="Aktivitas Desain dan Fotografi"
                            {{ $dataprofil->daftar_usaha_pariwisata == 'Aktivitas Desain dan Fotografi' ? 'selected' : '' }}>
                            Aktivitas Desain dan Fotografi</option>
                        <option value="Seni Pertunjukan"
                            {{ $dataprofil->daftar_usaha_pariwisata == 'Seni Pertunjukan' ? 'selected' : '' }}>Seni
                            Pertunjukan</option>
                        <option value="Fasilitas Olahraga"
                            {{ $dataprofil->daftar_usaha_pariwisata == 'Fasilitas Olahraga' ? 'selected' : '' }}>Fasilitas
                            Olahraga</option>
                    </select>
                ...
                <div class="form-group">
                    <label class=""
                        for="daftar_sub_jenis_usaha">{{ trans('cruds.dataprofil.fields.daftar_sub_jenis_usaha') }}</label>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" name="kecamatan" id="kecamatan">
                        ...
                        <option value="Bumiaji" {{ $dataprofil->kecamatan == 'Bumiaji' ? 'selected' : '' }}>Bumiaji
                        </option>
                        <option value="Junrejo" {{ $dataprofil->kecamatan == 'Junrejo' ? 'selected' : '' }}>Junrejo
                        </option>
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control time {{ $errors->has('start_time') ? 'is-invalid' : '' }}" type="time"
                        name="start_time" id="start_time" value="{{ old('start_time', $dataprofil->start_time) }}"
                        required>
...
@section('scripts')
    ...
    <script>
        ...
        onMapClick({
            latlng: L.latLng({{ $dataprofil->latitude }}, {{ $dataprofil->longitude }})
        });
        ...
        function getLocation() {
            if (navigator.geolocation) {
                navigator.geolocation.getCurrentPosition(function(position) {
                    ...
                    }); // Panggil fungsi onMapClick dengan lokasi GPS saat ini
                });
            } else {
                ...
                fetch(nominatimUrl)
                    ...
                    .then(function(data) {
                        if (data.length > 0) {
                            ...
                            onMapClick({
                                latlng: latlng
                            }); // Panggil fungsi onMapClick dengan koordinat alamat
                        } else {
    ...
    <script>
        ...
        var subJenisUsahaByDaftarUsahaPariwisata = {
            "Hotel dan Akomodasi Lainnya": ['Hotel Bintang', 'Hotel Melati', 'Villa', 'Penginapan Remaja',
                'Apartemen Hotel'
            ],
            // Opsi-opsi lainnya
            "Daya Tarik Wisata": ['Daya Tarik Wisata Alam', 'Daya Tarik Wisata Buatan/Binaan Manusia',
                'Daya Tarik Wisata Pemandian Alam'
            ],
            "Restoran": ['Restoran'],
            "Kafe": ['Kafe'],
            "Usaha Makanan": ['Data Rumah/Warung Makan', 'Kedai Makan', 'Penyedia Makanan keliling/ Tempat Tidak Tetap',
                'Restoran dan Penyedia Makanan Keliling Lainnya'
            ],
            ...
            "Jasa Pariwisata": ['Jasa Penyelenggara Pertemuan Perjalanan Insentif, Konferensi dan Pameran (MICE)',
                'Jasa Penyelenggara dataprofil Khusus (Special dataprofil)',
                'Jasa Boga Untuk Suatu dataprofil Tertentu (dataprofil Catering',
                'Jasa Pramuwisata', 'Jasa Informasi Daya Tarik Wisata (DTW)', 'Jasa Informasi Pariwisata',
                'Jasa Interpreter Wisata', 'Jasa Reservasi Lainnya YBDI YTDL', 'Aktivitas Konsultasi Pariwisata',
                'Penyedia Akomodasi Janhgka Pendek Lainnya', 'Penyedia Akomodasi Lainnya', 'Agen Perjalanan Wisata',
                'Biro Perjalanan Wisata', 'Data Angkutan Darat Wisata'
            ],
            "Perkemahan dan Pondok Wisata": ['Bumi Perkemahan, Persinggahan Karavan, dan Taman Karavan',
                'Pondok Wisata'],
            "Taman Rekreasi dan Hiburan": ['Taman Rekreasi', 'Aktivitas Taman Bertema atau Taman Hiburan Lainnya',
                'Usaha Area Permainan'
            ],
            "Aktivitas Desain dan Fotografi": ['Aktivitas Desain Kominikasi Visual/ Grafis',
                'Aktivitas Desain Khusus Film, Video, Program TV, Animasi dan Komik',
                'Aktivitas Desain Konten Kreatif Lainnya',
                'Aktivitas Perekaman Suara', 'Aktivitas Fotografi'
            ],
            "Seni Pertunjukan": ['Aktivitas Seni Pertunjukan', 'Pelaku Kreatif Seni Pertunjukan', 'Pelaku Seni Musik',
                'Aktivitas Hiburan, Seni dan Kreativitas Lainnya', 'Aktivitas Pekerja Kreatif Lainnya'
            ],
