                ...
                <div class="row mb-6">
                    <div class="col-md-5">
                        <div class="form-group">
                            ...
                            <input type="hidden" id="hidden_start_date" name="hidden_start_date" value="{{ old('start_date') }}">
                        </div>
                    </div>
                    <div class="col-md-5">
                        <div class="form-group">
                        ...
                        <tr data-entry-id="{{ $dataKunjungan->id }}">
                            ...
                            <td>
                                @can('data_kunjungan_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.data-kunjungans.show', $dataKunjungan->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('data_kunjungan_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.data-kunjungans.edit', $dataKunjungan->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('data_kunjungan_delete')
                                    <form action="{{ route('admin.data-kunjungans.destroy', $dataKunjungan->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@endcan
  ...
  let table = $('.datatable-DataKunjungan:not(.ajaxTable)').DataTable({ buttons: dtButtons })
