            ...
            <form method="POST" action="{{ route('admin.pakets.store') }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label class="required"
                        for="nama_paketWisata">{{ trans('cruds.paket.fields.nama_paketWisata') }}</label>
                    <input class="form-control {{ $errors->has('nama_paketWisata') ? 'is-invalid' : '' }}" type="text"
                        name="nama_paketWisata" id="nama_paketWisata" value="{{ old('nama_paketWisata', '') }}" required>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control ckeditor {{ $errors->has('deskripsi_paketWisata') ? 'is-invalid' : '' }}"
                        name="deskripsi_paketWisata" id="deskripsi_paketWisata">{!! old('deskripsi_paketWisata') !!}</textarea>
                ...
                <div class="form-group">
                    <label class="required"
                        for="gambar_paketWisata">{{ trans('cruds.paket.fields.gambar_paketWisata') }}</label>
                    <div class="needsclick dropzone {{ $errors->has('gambar_paketWisata') ? 'is-invalid' : '' }}"
                        id="gambar_paketWisata-dropzone">
                    </div>
                ...
                <div class="form-group">
                    ...
                    <div class="needsclick dropzone {{ $errors->has('pdf_paketWisata') ? 'is-invalid' : '' }}"
                        id="pdf_paketWisata-dropzone" accept=".pdf">
                    </div>
            ...
            init: function() {
                @if (isset($paket) && $paket->gambar_paketWisata)
                    ...
                    for (var i in files) {
                        ...
                        $('form').append('<input type="hidden" name="gambar_paketWisata[]" value="' + file
                            .file_name + '">')
                    }
                @endif
            },
                ...
                init: function() {
                    @if (isset($paket) && $paket->pdf_paketWisata)
                        ...
                        for (var i in files) {
                            ...
                            this.options.thumbnail.call(this, file, file.preview ?? file.preview_url);
                            file.previewElement.classList.add('dz-complete');
                            $('form').append('<input type="hidden" name="pdf_paketWisata[]" value="' + file
                                .file_name + '">');
                        }
                    @endif
                },
